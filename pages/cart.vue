      ...
      >
        <div class="flex items-center">
          <van-checkbox
            class="mr-1"
            v-model="shop.check"
            @click="handleShopCheckChange(shop)"
          ></van-checkbox>
        ...
        <ul>
          ...
          >
            <van-checkbox
              ...
              @change="handleProductCheckChange(shop)"
            ></van-checkbox>
            <van-image
              class="mr-1 rounded-sm overflow-hidden"
              width="80"
              height="80"
              fit="cover"
...
export default Vue.extend({
  ...
  mounted() {},
  methods: {
    handleShopCheckChange(shop: {
      check: boolean
      product: [{ check: boolean }]
    }) {
      shop.product.map((item) => {
        item.check = shop.check
      })
    },
    handleProductCheckChange(shop: {
      check: boolean
      product: [{ check: boolean }]
    }) {
